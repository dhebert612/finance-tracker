...
const payFrequencyEnum = z.enum(['weekly', 'biweekly', 'semi_monthly', 'monthly']);
...
export const incomeSourceHandler = {
  ...
  async getOne(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    ...
    if (!source) return reply.status(404).send({ error: 'Income source not found' });
  ...
  async remove(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    ...
    if (!deleted) return reply.status(404).send({ error: 'Income source not found' });
